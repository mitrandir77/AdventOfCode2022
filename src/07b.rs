...
fn main() -> Result<()> {
    ...
    let total_size = walk(&dir_tree, &mut result, &cwd);
    let free = 70000000 - total_size;
    let free_needed = 30000000;
    let missing_free = max(free_needed as i32 - free as i32, 0) as usize;

    let mut min_viable = 70000000;
    for val in result.values() {
        if *val >= missing_free {
            min_viable = min(min_viable, *val);
        }
    }
    println!("{}", min_viable);
    Ok(())
